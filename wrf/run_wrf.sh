...
export DOMAIN_BASE=$BASE_PATH/domains/mocoa
...
#
cat << Fin | sed -e 's/#.*//; s/  *$//' > $DOMAIN_BASE/namelist.wps_${anyoinicial[$i]}${mesinicial[$i]}${diainicial[$i]}
...
&geogrid
 ...
 truelat2  = 0,
 stand_lon = -74.411,
 geog_data_path = '/global/scratch/users/ejhernandezb/geo_data/WPS_GEOG/',
 opt_geogrid_tbl_path = '/global/scratch/users/ejhernandezb/WPS/geogrid/',
 ref_x = 131.0,
 ref_y = 177.5,
/
...
#Ahora, copio el namelist generado al que se utiliza en la corrida
 ...
 rm -v Vtable
 rm -v GRIBFILE.A*
 ln -sf $BASE_PATH/WPS/ungrib/Variable_Tables/Vtable.GFS Vtable
# ./link_grib.csh $BASE_PATH/data_wrf/gfsanl_
./link_grib.csh $BASE_PATH/data_wrf/gfs.* # posición de los datos
...
while [ $j -lt 11 ]; do
 ...
 cat << Fin | sed -e 's/#.*//; s/  *$//' > /global/scratch/users/ejhernandezb/domains/mocoa/namelist.input${anyoinicial[$((i))]}${mesinicial[$((i))]}${diainicial[$i]}Ens_${miembro[$j]}
...
Fin
 ...
 cd $DOMAIN_BASE
 cp $BASE_INSTALLER/WPS/metgrid/METGRID.TBL .
 cp $BASE_INSTALLER/WRF/run/LANDUSE.TBL .
 cp $BASE_INSTALLER/WRF/run/RRTM_DATA .
 cp $BASE_INSTALLER/WRF/run/VEGPARM.TBL .
 cp $BASE_INSTALLER/WRF/run/SOILPARM.TBL .
 cp $BASE_INSTALLER/WRF/run/GENPARM.TBL .
