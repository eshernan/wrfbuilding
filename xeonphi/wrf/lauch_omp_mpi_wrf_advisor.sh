...
MPI_NUM_PROCESS=2
DOMAIN_DIST=numa
HOME_EXECW=/data/bin/3.8/wrf_advisor
...
OUTPUT_DIR=performance_$MPI_RANKS"_"$OMP_NUM_THREADS_"_$(date +"%H_%M_%S")"
...
#mpirun -print-rank-map -machine $HOSTS  -np $MPI_RANKS -env I_MPI_PIN_DOMAIN $DOMAIN_DIST amplxe-cl -trace-mpi -result-dir performance_$MPI_RANKS"_"$OMP_NUM_THREADS -collect hpc-performance  -data-limit=2000 $HOME_EXECW/wrf.exe 
#mpirun -print-rank-map -machine $HOSTS  -np $MPI_RANKS -env I_MPI_PIN_DOMAIN $DOMAIN_DIST $HOME_EXECW/wrf.exe 
#amplxe-cl -result-dir $OUTPUT_DIR -collect advanced-hotspots -knob enable-user-tasks=true -knob analyze-openmp=true  -data-limit=2000 $HOME_EXECW/wrf.exe 
