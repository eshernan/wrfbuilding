...
######For profiling performance on one node, 
mpirun -gtool "amplxe-cl -collect hpc-performance -r result:0" -print-rank-map -machine $HOSTS  -np $MPI_RANKS -env I_MPI_PIN_DOMAIN $DOMAIN_DIST $HOME_EXECW/wrf.exe 
